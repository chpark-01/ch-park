...
for h in range(40):
    ecg_data = df1.iloc[:, h]  # read subject’s raw data
...
shift_data = normal_data + (shift - mean_data)  # shift baseline
shift_data = shift_data.tolist()
normal_full.append(shift_data)

shifted_normal = [item for sublist in normal_full for item in sublist]

for it in range(img_num):
    data_crop = shifted_normal[(it * fs):(it * fs) + round(second_num * fs) - 1]

    # save img (set your file path in ‘path’)
    file_name1 = path1.split('admin')[0] + 'path' + str(h) + '_' + str(it) + '.png'
    print(file_name1)
    print(path1.split('.'))
    print(str(h + 1) + '번째 signal iter:' + str(it))
    signal_plot(data_crop, fs, file_name1)

    file_name2 = path1.split('admin')[0] + 'path' + str(h) + '_' + str(it) + '.png'
    print(file_name2)
    print(path1.split('.'))
    print(str(h + 1) + '번째 stft iter:' + str(it))
